...
const MembersPanel = ({ projectId, onClose }) => {
  ...
  const handleRemoveMember = async (userId, name) => {
    if (window.confirm(t('projects.members.removeConfirm', { name }))) {
      try {
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between border-b border-border p-6">
          <h2 className="text-xl font-semibold text-fg">{t('projects.members.title')}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <h4 className="mb-4 text-[13px] font-semibold uppercase text-muted">
            {t('projects.members.current', { count: members.length })}
          </h4>
                ...
                <div key={memberId} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="truncate text-sm font-medium text-fg">
                      {member.name}{' '}
                      {isOwner(memberId) && (
                        <span className="ml-1 text-[11px] text-priority-high">
                          <span aria-hidden="true">👑 </span>{t('projects.members.owner')}
                        </span>
                      )}
                    </div>
          ...
          <div className="mt-8 border-t border-border pt-6">
            <h4 className="mb-4 text-[13px] font-semibold uppercase text-muted">{t('projects.members.addSection')}</h4>
            <input
              type="text"
            ...
            <div className="mt-3 flex flex-col gap-2">
              {isSearching && <div className="p-2 text-center text-[13px] text-muted">{t('projects.members.searching')}</div>}
              {!isSearching && search.trim() && searchResults.length === 0 && (
                <div className="p-2 text-center text-[13px] text-muted">{t('projects.members.noAvailable')}</div>
              )}
              {!isSearching && searchResults.map((u) => (
                ...
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    ...
                    <div className="truncate text-[11px] text-muted">{u.email} • {userRoleLabel(u.role)}</div>
                  </div>
                  <div className="text-xs font-semibold text-accent">{t('projects.members.add')}</div>
                </div>
              ))}
            </div>
          </div>
